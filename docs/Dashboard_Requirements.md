...
| Run history | Maintain at least the last 10 runs via the persistence layer (`FileSystemPipelineRunRepository`) so regressions can be compared quickly. Run history persists across server restarts. |
...
4. Developers can restart the server and the dashboard still functions (run history persists via `FileSystemPipelineRunRepository`).
