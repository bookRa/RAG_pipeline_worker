...
| Stage inspection | Dashboard renders one numbered card per stage (ingestion, extraction, cleaning, chunking, enrichment, vectorization) with the serialized payload (page previews, chunk counts, cleaning stats, vectors). |
...
1. **In-app experience** – dashboard ships with FastAPI (no extra Node/React project). Uses Jinja + HTMX for partial updates.
...
3. The dashboard handles unsupported files gracefully (400 error rendered by HTMX).
